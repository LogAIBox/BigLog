...
@dataclass
class DataCollatorForLanguageModeling_for_biglog(DataCollatorMixin):
    """
    ...
    def tf_call(self, examples: List[Union[List[int], Any, Dict[str, Any]]]) -> Dict[str, Any]:
        ...
        if self.mlm:
            ...
            batch["labels"] = labels
        return batch
    def get_special_tokens_mask_biglog(
    ...
    def torch_mask_tokens(self, inputs: Any, special_tokens_mask: Optional[Any] = None) -> Tuple[Any, Any]:
        ...
        if special_tokens_mask is None:
            special_tokens_mask = [
                self.get_special_tokens_mask_biglog(val, already_has_special_tokens=True) for val in labels.tolist()
            ]
...
@dataclass
class DataCollatorForTermMask(DataCollatorForLanguageModeling_for_biglog):
    ...
    def torch_call(self, examples: List[Union[List[int], Any, Dict[str, Any]]]) -> Dict[str, Any]:
        ...
        mask_labels = []
        assert('term_refs') in examples[0]
        for e in examples:
            ref_tokens = []
            ...
            # For term tokens, we need extra info to mark
            ref_pos = tolist(e["term_refs"])
            # len_seq = len(e["input_ids"])
            # for i in range(len_seq):
            #     if i in ref_pos:
            #         ref_tokens[i] = "##" + ref_tokens[i]
            mask_labels.append(self.term_mask(ref_tokens,ref_pos))
        batch_mask = _torch_collate_batch(mask_labels, self.tokenizer, pad_to_multiple_of=self.pad_to_multiple_of)
        inputs, labels = self.torch_mask_tokens(batch_input, batch_mask)
        return {"input_ids": inputs, "labels": labels}
    def term_mask(self, input_tokens,ref_pos, max_predictions=512):
        """
        ...
        for (i, token) in enumerate(input_tokens):
            ...
            if len(cand_indexes) >= 1 and i in ref_pos:
    ...
    def torch_mask_tokens(self, inputs: Any, mask_labels: Any) -> Tuple[Any, Any]:
        ...
        return inputs, labels
@dataclass
class DataCollatorForWholeWordMask(DataCollatorForLanguageModeling_for_biglog):
    """
...
@dataclass
class DataCollatorForSOP(DataCollatorForLanguageModeling_for_biglog):
    """
